...
def train_parseq(args):
    if args.dataset == 'Hockey':
        ...
        command = f"conda run -n {config.str_env} {config.python_executable} train.py +experiment=parseq dataset=real data.root_dir={data_root} trainer.max_epochs=25 " \
...
def hockey_pipeline(args):
    ...
    if args.pipeline['legible']:
        ...
        command = f"{config.python_executable} legibility_classifier.py --data {root_dir} --arch resnet34 --trained_model {config.dataset['Hockey']['legibility_model']}"
        success = os.system(command) == 0
    ...
    if success and args.pipeline['str']:
        ...
        current_dir = os.getcwd()
        data_root = os.path.join(current_dir, config.dataset['Hockey']['root_dir'], config.dataset['Hockey']['numbers_data'])
        command = f"conda run -n {config.str_env} {config.python_executable} str.py  {config.dataset['Hockey']['str_model']}\
            --data_root={data_root}"
        success = os.system(command) == 0
...
def soccer_net_pipeline(args):
    ...
    timings_base = os.path.join(config.dataset['SoccerNet']['working_dir'],config.dataset['SoccerNet'][args.part]['timestamps_path'])
    if not os.path.isdir(timings_base):
        os.makedirs(timings_base)

    completes = 0
    for name in os.listdir(timings_base):
        if 'run_' in name:
            completes += 1

    iteration = len(os.listdir(timings_base))
    timings_complete_path = os.path.join(timings_base,f'run_{completes}.json')
    timings_path = os.path.join(timings_base,f'time_{completes}.json')

    timings = { key: 0 for key in args.pipeline }
    ...
    if args.pipeline['soccer_ball_filter']:
        ...
        timings['soccer_ball_filter'] = timer.get_elapsed()
    ...
    if args.pipeline['feat']:
        with timer:
            ...
            command = f"conda run --live-stream -n {config.reid_env} {config.python_executable} {config.reid_script} --tracklets_folder {image_dir} --output_folder {features_dir}"
            success = helpers.run_subprocess(command)
            print(success)
            print("Done generating features")
        timings['feat'] = timer.get_elapsed()
        with open(timings_path,'w') as f:
            json.dump(timings, f)
    ...
    if args.pipeline['filter'] and success:
        with timer:
            ...
            command = f"{config.python_executable} gaussian_outliers.py --tracklets_folder {image_dir} --output_folder {features_dir}"
            success = helpers.run_subprocess(command)
            print("Done removing outliers")
        timings['filter'] = timer.get_elapsed()
        with open(timings_path,'w') as f:
            json.dump(timings, f)
    ...
    if args.pipeline['legible'] and success:
        ...
        timings['legible'] = timer.get_elapsed()
        with open(timings_path,'w') as f:
            json.dump(timings, f)
    ...
    if args.pipeline['legible_eval'] and success:
        ...
        timings['legible_eval'] = timer.get_elapsed()
        with open(timings_path,'w') as f:
            json.dump(timings, f)

    #4. generate json for pose-estimation
    if args.pipeline['generate-pose'] and success:
        with timer:
        ...
        timings['generate-pose'] = timer.get_elapsed()
        with open(timings_path,'w') as f:
            json.dump(timings, f)
    ...
    if args.pipeline['pose'] and success:
        ...
        with timer:
            ...
            command = f"conda run --live-stream -n {config.pose_env} {config.python_executable} pose.py {config.pose_home}/configs/body/2d_kpt_sview_rgb_img/topdown_heatmap/coco/ViTPose_huge_coco_256x192.py \
                {config.pose_home}/checkpoints/vitpose-h.pth --img-root / --json-file {input_json} \
                --out-json {output_json}"
            success = helpers.run_subprocess(command)
            print("Done detecting pose")
        timings['pose'] = timer.get_elapsed()
        with open(timings_path,'w') as f:
            json.dump(timings, f)
    ...
    if args.pipeline['crops'] and success:
        ...
        timings['crops'] = timer.get_elapsed()
        with open(timings_path,'w') as f:
            json.dump(timings, f)
    ...
    if args.pipeline['str'] and success:
        with timer:
            ...
            command = f"conda run --live-stream -n {config.str_env} {config.python_executable} str.py  {config.dataset['SoccerNet']['str_model']}\
                --data_root={image_dir} --batch_size=1 --inference --result_file {str_result_file}"
            success = helpers.run_subprocess(command)
            print("Done predict numbers")
        timings['str'] = timer.get_elapsed()
        with open(timings_path, 'w') as f:
            json.dump(timings, f)
    ...
    if args.pipeline['combine'] and success:
        ...
        timings['combine'] = timer.get_elapsed()
        with open(timings_path, 'w') as f:
            json.dump(timings, f)

    if args.pipeline['eval'] and success:
        #9. evaluate accuracy
        shutil.copyfile(timings_path, timings_complete_path)
        with open(timings_complete_path, 'r') as f:
            timings = json.load(f)
        ...
        helpers.evaluate_results(consolidated_dict, gt_dict, full_results = analysis_results, timings = timings)
