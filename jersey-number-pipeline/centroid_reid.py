...
def generate_features(input_folder, output_folder, model_version='res50_market'):
    ...
    print(f"Loading model from {MODEL_FILE} {use_cuda and 'using GPU'}")
    ...
    model = CTLModel.load_from_checkpoint(cfg.MODEL.PRETRAIN_PATH, cfg=cfg, weights_only=False, use_cuda=use_cuda)
    if use_cuda:
        model.to('cuda')
    model.eval()
