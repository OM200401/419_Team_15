...
def evaluate_results(consolidated_dict, gt_dict, full_results = None, timings = None):
    correct = 0
    total = 0
    incorrect = 0
    mistakes = []
    ...
    print(f"Total number of tracklets: {total}\nCorrect: {correct}\nAccuracy: {100.0 * correct/total}%")
    ...
    if timings:
        for k,v in timings.items():
            timestr = f"{v*1000:.2f} milliseconds"
            if v >= 3600:
                timestr = f"{v/3600:.2f} hours"
            elif v >= 60:
                timestr = f"{v/60:.2f} minutes"
            elif v >= 1:
                timestr = f"{v:.2f} seconds"
            print(f"Step '{k}' took {timestr}")
...
def run_subprocess(command: str) -> bool:
    process = None
    try:
        process = subprocess.Popen(command, stdout=subprocess.PIPE)
        for c in iter(lambda: process.stdout.read(1), b""):
            sys.stdout.buffer.write(c)
        process.communicate() # ensures 'returncode' property is populated (indirect)
    except KeyboardInterrupt:
        process.kill()
        return False
    return process and process.returncode == 0
